...
export default function Reviews() {
  ...
  return (
    <Layout>
      <Seo
        pageTitle={reviewsPage.title}
        description={reviewsPage.shareExperience.description}
      />
        ...
        <div className="container-wide text-center">
          ...
          <AnimateOnScroll animation="scale" delay={100}>
            <div className="flex items-center justify-center gap-2 mb-2">
              {[1, 2, 3, 4, 5].map((i) => (
                <Star key={i} className={`w-6 h-6 ${hasReviews ? "fill-warning text-warning" : "text-muted"}`} />
              ))}
            </div>
          </AnimateOnScroll>
            ...
            <AnimateOnScroll animation="fade-up" delay={300}>
              <div className="glass-card p-8 rounded-3xl text-center mb-12 max-w-3xl mx-auto border border-white/20 shadow-xl relative overflow-hidden">
                ...
                <Button asChild size="lg" className="rounded-full px-8 btn-hover-scale shadow-lg shadow-primary/20">
                  <ContentLink href={reviewFormHref} newTabForExternal>
          ...
          {filteredReviews.length === 0 ? (
            <AnimateOnScroll animation="fade-up">
              <div className="glass-card rounded-2xl p-8 text-center">
                ...
                <p className="text-muted-foreground">
                  Verified patient feedback will appear here after you add entries in `content/reviews.txt`.
                </p>
              </div>
            </AnimateOnScroll>
          ) : (
